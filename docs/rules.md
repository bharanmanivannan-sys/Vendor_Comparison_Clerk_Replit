...
18. Accept one concrete entity plus an explicit competitor objective such as “other e-commerce sites.” Preserve the named entity and resolve up to three objective slots into concrete local alternatives, producing a shortlist of up to four options before analysis.
19. When official claims cannot separate options, a review-signal winner requires comparable retrieved review evidence for every option. Use multiple recent independent sources, credibility, balanced detail, structured ratings, sample size, and incentive risk; otherwise return no exact winner.
