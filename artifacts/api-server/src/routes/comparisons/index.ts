  ...
  void (async () => {
    ...
    try {
      const analysis = await buildAnalysis({
        ...
        onEntitiesDiscovered: (entities) => {
          options.vendors.splice(0, options.vendors.length, ...entities.slice(0, MAX_COMPARISON_OPTIONS));
          const current = comparisonJobs.get(id);
          if (current?.status === "processing") {
            comparisonJobs.set(id, {
              ...current,
...
export async function validateComparisonInput(
  ...
  const canonicalVehiclePromptVendors = hasProvidedVendors
    ...
    && parsedPrompt.vendors.length === input.vendors?.length
    && /\b(?:car|cars|vehicle|vehicles|diesel|petrol|suv)\b/i.test(input.prompt)
    ? parsedPrompt.vendors
    : undefined;
  const vendors = hasProvidedVendors
    ? canonicalVehiclePromptVendors ?? input.vendors as string[]
    : parsedPrompt.vendors;
...
export function buildComparisonDecisionSet(comparison: {
  ...
  const vendors = finalScoredVendors.length === submittedVendors.length
