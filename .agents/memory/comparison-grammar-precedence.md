...
Treat vendor extraction as a precedence problem, not a collection of phrase-specific exceptions. Explicit `between` pairs outrank earlier descriptive lists; explicit chosen or comma-separated lists outrank incidental pair matches; other clear comparison pairs outrank broad prepositional clauses. “Models from A and B available…” is an explicit manufacturer list, not a generic comparison pair. Known EV brands joined only by `/`, `&`, commas, `and`, `or`, `vs`, `versus`, or `against` are distinct options.
